...
function MenuState() {
  ...
  this.setup = function() {
    ...
    title_anim = new jaws.Animation({sprite_sheet: "./assets/art/title_animation.png", frame_size:[874,579],loop:true});
    title_img  = new jaws.Sprite({x:jaws.width/2, y:jaws.height/2, anchor:"center"});
  	...
  	if(lightbulb_on) {
		var rand = random_sigmoid();
  		...
  		if(rand > 0.8 || rand < -0.8) {
  			...
  			lightbulb_on = false;
  			title_img.setImage(title_anim.next());
  			setTimeout(function(){lightbulb_on = true;},750*rand);
  		}  		
  	} 
  }
